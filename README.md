...
- 新增 SQLite 持久化使用数据，性能更佳
- 配置支持模型价格配置
- **重要：**建议配合[Fork版WebUI](https://github.com/ZhuofanWu/Cli-Proxy-API-Management-Center)食用。
- 测试环境仅有Windows exe，不对其他版本稳定性做保证。docker-compose.yml 里默认使用镜像 eceasy/cli-proxy-api:latest，**并非本项目的镜像**！
