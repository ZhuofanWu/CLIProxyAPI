...
func (m configTabModel) parseConfig(cfg map[string]any) []configField {
	...
	fields = append(fields, configField{"Usage Statistics Storage Way", "usage-statistics-storage-way", "string", getString(cfg, "usage_statistics_storage_way"), nil})
...
func fieldSection(apiPath string) string {
	...
	case "logging-to-file", "logs-max-total-size-mb", "error-logs-max-files", "usage-statistics-storage-way", "usage-statistics-enabled", "request-log":
